...
def mutate(model_file, malicious_file, evasive_output):
    ...
    for sample_num, malicious_sample in enumerate(malicious_samples):
        ...
        for i in range(5000):
            ...
            best_feat = range(2,max_feat)[best_feat_index]
            ...
            if best_probs[1] < 0.5:
                std_logger.warning("Success | Final: " + str(best_probs[1]))
                evasive_file.write(malicious.sparse_arff())
                print("Success - final prob: " + str(best_probs[1]))
                break

            # print("Progress: " + str(i))

        if best_probs[1] > 0.5:
            std_logger.warning("Failure | Final prob: " + str(best_probs[1]))
            print("Failure | Final prob: " + str(best_probs[1]))
